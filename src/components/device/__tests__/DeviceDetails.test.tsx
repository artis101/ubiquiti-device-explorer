...
vi.mock('../details/DeviceDetailsHeader', () => ({
  DeviceDetailsHeader: () => <div data-testid="device-details-header">Header</div>,
}));
...
vi.mock('../details/DeviceJsonModal', () => ({
  DeviceJsonModal: ({ device, onClose }: { device: NormalizedDevice; onClose: () => void }) => (
...
describe('DeviceDetails', () => {
  ...
  it('should show JSON modal when "See All Details as JSON" is clicked', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('JSON Modal for Test Device')).toBeInTheDocument();
    });
  });
