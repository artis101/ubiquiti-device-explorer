...
describe('searchDevices', () => {
  ...
  it('should find devices by displayName', () => {
    const results = searchDevices(mockDevices as NormalizedDevice[], 'Device A');
  ...
  it('should find devices by product name', () => {
    const results = searchDevices(mockDevices as NormalizedDevice[], 'Product C');
    expect(results.length).toBe(1);
  ...
  it('should return empty array if no matches', () => {
    const results = searchDevices(mockDevices as NormalizedDevice[], 'NonExistent');
...
describe('filterByLine', () => {
  ...
  it('should return all devices if lineId is undefined', () => {
    expect(filterByLine(mockDevices as NormalizedDevice[], undefined)).toEqual(mockDevices);
  });
...
describe('filterByProductLines', () => {
  ...
  it('should return all devices if productLines is empty or undefined', () => {
    expect(filterByProductLines(mockDevices as NormalizedDevice[], [])).toEqual(mockDevices);
    expect(filterByProductLines(mockDevices as NormalizedDevice[], undefined as unknown as string[])).toEqual(mockDevices);
  });

  it('should filter devices by multiple product lines', () => {
    const results = filterByProductLines(mockDevices as NormalizedDevice[], ['line1', 'line3']);
    expect(results.length).toBe(3);
    expect(results.some(d => d.id === '2')).toBe(false);
  });

  it('should return empty array if no devices match any product line', () => {
    const results = filterByProductLines(mockDevices as NormalizedDevice[], ['line4']);
    expect(results.length).toBe(0);
  });
});
