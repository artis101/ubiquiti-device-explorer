...
describe('normalizeDevices', () => {
  ...
  it('should collect warnings for invalid or incomplete device objects', () => {
    ...
    const invalidObjectWarnings = warnings.filter(w => w.reason === 'Invalid or incomplete device object');
  ...
  it('should collect warnings for duplicate device IDs', () => {
    ...
    const duplicateWarnings = warnings.filter(w => w.reason === 'Duplicate device ID (first occurrence kept)');
  ...
  it('should collect warnings for missing product name and shortnames', () => {
    ...
    const missingNameWarnings = warnings.filter(w => w.reason === 'Missing product name and shortnames');
    expect(missingNameWarnings.length).toBe(1);
    
    // Check for the missing images warning
    const missingImagesWarnings = warnings.filter(w => w.reason === 'Missing all image hashes');
